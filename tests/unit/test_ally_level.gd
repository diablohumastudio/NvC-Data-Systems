...
# Test for the AllyLevelData class
...
func test_ally_level_creation():
	# Test that we can create an AllyLevelData
	var ally_level = AllyLevelData.new()
	assert_not_null(ally_level, "AllyLevelData should be created successfully")
...
func test_ally_level_has_level_id_property():
	...
	assert_eq(ally_level.level_id, "fast_chest_lvl_1", "AllyLevelData level_id should be settable and gettable")

func test_ally_level_has_price_property():
	# Test the price property
	var ally_level = AllyLevelData.new()
	ally_level.price = 100
	assert_eq(ally_level.price, 100, "AllyLevelData price should be settable and gettable")

# Inherited Property Tests
func test_ally_level_inherits_initial_hp():
	# Test that AllyLevelData inherits initial_HP from GameCharacterLevel
	var ally_level = AllyLevelData.new()
	ally_level.initial_HP = 50
	assert_eq(ally_level.initial_HP, 50, "AllyLevelData should inherit initial_HP property")
