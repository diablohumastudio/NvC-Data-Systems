...
func _on_go_to_main_menu_button_pressed():
	$WoodenPopUp/GoToMainMenuButton/AnimationPlayer.play("pressed")
	
	var main_menu : PackedScene = load(GC.SCREENS_UIDS.MAIN_MENU)
	
	AudioSystem.post_event(AK.EVENTS.PLAY_BUTTON_MENU_CLICK2)
	await $WoodenPopUp/GoToMainMenuButton/AnimationPlayer.animation_finished
	
	get_tree().paused = false
	SMS.change_scene(main_menu)

func _on_play_again_button_pressed():
	$WoodenPopUp/PlayAgainButton/AnimationPlayer.play("pressed")
	var game : PackedScene = load(GC.SCREENS_UIDS.GAME)
	
	AudioSystem.post_event(AK.EVENTS.PLAY_BUTTON_MENU_CLICK2)
	await $WoodenPopUp/PlayAgainButton/AnimationPlayer.animation_finished
