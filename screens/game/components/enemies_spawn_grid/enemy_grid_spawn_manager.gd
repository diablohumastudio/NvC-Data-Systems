...
@export var enemies_spawners_grid: EnemiesSpawnersGrid
...
func _ready() -> void:
	...
	get_parent().ready.connect(func():spawn_wave(wave))
...
func spawn_wave(wave: WaveData):
	...
	var spawn_grid_positions: Array[Vector2] = wave.position_strategy.get_spawning_positions(total_enemies_number, enemies_spawners_grid)
	...
	for ii in total_enemies_number:
		...
		var enemies_spawner: EnemiesSpawnerCell = enemies_spawners_grid.get_cell_by_col_and_row(position.x, position.y)
