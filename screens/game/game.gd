...
func _update_kill_enemy_label():
	%KillEnemyLbl.text = str(%GameStats.enemies_killed) + " Enms Klld in lvl"
...
func _on_kill_enemy_btn_pressed() -> void:
	...
	ACS.set_action(Action.new(Action.TYPES.ENEMY_KILLED, Action.PayEnemKilled.new(%GameStats.enemies_killed)))
	_update_kill_enemy_label()

func _on_canon_killed_btn_pressed() -> void:
	%GameStats.canons_alive -= 1
	%CanonsLeft.text = str(%GameStats.canons_alive) + " Canons Left"
