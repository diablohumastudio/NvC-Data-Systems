...
func _on_accept_btn_pressed() -> void:
	...
	var can_create_user: UDS.CAN_CREATE_USER_ERRORS = UDS.can_create_new_user(new_credentials)
	if can_create_user == UDS.CAN_CREATE_USER_ERRORS.MAX_NUMBER_OF_USER_CREATED:
		%WarningLabel.text = "MAXIMUN number of users created"
	elif can_create_user == UDS.CAN_CREATE_USER_ERRORS.USER_SAME_NAME:
		%WarningLabel.text = "SAME NAME user already created"
	elif can_create_user == UDS.CAN_CREATE_USER_ERRORS.OK:
