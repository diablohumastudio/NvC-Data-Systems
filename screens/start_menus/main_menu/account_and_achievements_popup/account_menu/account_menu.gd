...
signal delete_user_btn_pressed
...
func _on_username_text_edit_text_changed() -> void:
	current_user_credential.user_name =  %UsernameLabel.text

func _on_new_user_btn_pressed() -> void:
	create_new_user_btn_pressed.emit()

func _on_delete_user_btn_pressed() -> void:
	delete_user_btn_pressed.emit()

func _set_visuals():
	%UsernameLabel.text = current_user_credential.user_name
