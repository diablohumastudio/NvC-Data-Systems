...
signal ally_btn_selected(ally_btn:AllyBtn)
...
func populate_container(allies:Array[AllyData]) -> void:
	...
	for ally in allies:
		...
		new_ally_btn.pressed.connect(_on_ally_btn_selected.bind(ally))
		
	%ButtonsContainer.get_child(0).button_pressed = true # Pending to move

func _on_ally_btn_selected(ally_btn:AllyBtn) -> void:
